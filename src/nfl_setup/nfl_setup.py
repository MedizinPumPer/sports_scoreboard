...
from src.nfl_api.data import get_nfl_teams
from print import *
import argparse
import functools
import regex
...
def save_config(nfl_config,confdir):
    savefile = json.dumps(nfl_config, sort_keys=False, indent=4)
    ...
    refresh_rate = [
        {
            ...
            'qmark': qmark,
            'message': 'Live game refresh rate? (rate at which a live game will call the NFL API to catch the new data, 10 seconds is the lowest)',
    ...
    if use_ss['enabled']:
        ...
        ss_questions = [
            ...
            },
            {
                ...
                'name': 'data_updates',
                'qmark': qmark,
                'message': 'Update data feeds while screensaver is on? (applies to all weather feeds and dimmer, nfl will always be blocked with screensaver on)',
...
def main():
    ...
    if not args.simple:
        print("NFL LED SCOREBOARD SETUP", SMSLANT,RED, BOLD)
        setupVersion="nfl_setup V{}".format(SCRIPT_VERSION)
        print(setupVersion,UNDERLINE,BLUE)
        mainVersion="nfl led scoreboard V{}".format(getVersion())
    ...
    if questionary.confirm("Save {}/config.json file?".format(args.confdir),qmark=qmarksave,style=custom_style_dope).ask():
        save_config(nfl_config,args.confdir)
