...
class LogoRenderer:
    def __init__(self, matrix, config, element_layout, team_abbrev, board, gameLocation=None):
        ...
        self.logo_variant = config.config.logos.get_team_logo(team_abbrev.logo)
    ...
    def load(self, team_abbrev):
        try:
            filename = self.get_path(team_abbrev.abbrev)
    ...
    def save_image(self, filename, team_abbrev):
        if not os.path.exists(os.path.dirname(filename)):
            try:
                ...
                self.change_ownership(team_abbrev.abbrev)

            except OSError as exc:  # Guard against race condition
                if exc.errno != errno.EEXIST:
                    raise
        try:
            self.logo = Image.open(requests.get(team_abbrev.logo, stream=True).raw)
        except:
            self.logo = ImageHelper.image_from_svg(
                LOCAL_LOGO_URL.format(team_abbrev.abbrev, self.logo_variant)
            )
